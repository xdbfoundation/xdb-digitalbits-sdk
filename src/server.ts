...
export class Server {
  ...
  constructor(serverURL: string, opts: Server.Options = {}) {
    ...
    if (!isEmpty(customHeaders)) {
      FrontierAxiosClient.interceptors.request.use((config) => {
   ...
   * your machine's local time could be different from Frontier's. This gives you
   ...
   *
   * ```javascript
   * const transaction = new DigitalBitsSdk.TransactionBuilder(accountId, {
   *  fee: await DigitalBitsSdk.Server.fetchBaseFee(),
   *  timebounds: await DigitalBitsSdk.Server.fetchTimebounds(100)
   * })
   ...
   * This is to avoid a scenario where Frontier is horking up the wrong date.
  ...
  public async fetchTimebounds(
    ...
    // FrontierAxiosClient instead of this.ledgers so we can get at them headers
    ...
    await FrontierAxiosClient.get(URI(this.serverURL as any).toString());
  ...
  public async feeStats(): Promise<Frontier.FeeStatsResponse> {
    const cb = new CallBuilder<Frontier.FeeStatsResponse>(
  ...
  public async submitTransaction(
    ...
    return FrontierAxiosClient.post(
      URI(this.serverURL as any)
  ...
  /**
   * @returns {AccountCallBuilder} New {@link AccountCallBuilder} object configured by a current Frontier server configuration.
   */
  ...
  /**
   * @returns {ClaimableBalanceCallBuilder} New {@link ClaimableBalanceCallBuilder} object configured by a current Frontier server configuration.
   */
  ...
  /**
   * @returns {LedgerCallBuilder} New {@link LedgerCallBuilder} object configured by a current Frontier server configuration.
   */
  ...
  /**
   * @returns {TransactionCallBuilder} New {@link TransactionCallBuilder} object configured by a current Frontier server configuration.
   */
  ...
  /**
   * People on the DigitalBits network can make offers to buy or sell assets. This endpoint represents all the offers on the DEX.
   *
   ...
   * @returns {OrderbookCallBuilder} New {@link OrderbookCallBuilder} object configured by a current Frontier server configuration.
   */
  ...
  /**
   * Returns
   * @returns {TradesCallBuilder} New {@link TradesCallBuilder} object configured by a current Frontier server configuration.
   */
  ...
  /**
   * @returns {OperationCallBuilder} New {@link OperationCallBuilder} object configured by a current Frontier server configuration.
   */
  ...
  /**
   * The DigitalBits Network allows payments to be made between assets through path
   ...
   *
   * As part of the search, frontier will load a list of assets available to the
   ...
   * @returns {StrictReceivePathCallBuilder} New {@link StrictReceivePathCallBuilder} object configured with the current Frontier server configuration.
   */
  ...
  /**
   * The DigitalBits Network allows payments to be made between assets through path payments. A strict send path payment specifies a
   ...
   * @returns {StrictSendPathCallBuilder} New {@link StrictSendPathCallBuilder} object configured with the current Frontier server configuration.
   */
