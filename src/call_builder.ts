...
import FrontierAxiosClient from "./frontier_axios_client";
import { ServerApi } from "./server_api";

/* tslint:disable-next-line:no-var-requires */
const version = require("../package.json").version;

// Resources which can be included in the Frontier response via the `join`
// query-param.
 ...
 * @param {string} serverUrl URL of Frontier server
 * @class CallBuilder
 */
export class CallBuilder<
  T extends
    | Frontier.FeeStatsResponse
    | Frontier.BaseResponse
    | ServerApi.CollectionPage<Frontier.BaseResponse>
> {
  protected url: URI;
  ...
  public stream(options: EventSourceOptions<T> = {}): () => void {
    ...
    const createEventSource = () => {
      ...
      if (es) {
        // when receiving the close message from Frontier we should
   ...
   */
  private _requestFnForLink(link: Frontier.ResponseLink): (opts?: any) => any {
    ...
    for (const key of Object.keys(json._links)) {
      ...
      */
      if (included && JOINABLE.indexOf(key) >= 0) {
        ...
        json[key] = this._requestFnForLink(n as Frontier.ResponseLink);
      }
    }
    return json;
  }
  ...
  private async _sendNormalRequest(initialUrl: URI) {
    ...
    return FrontierAxiosClient.get(url.toString())
